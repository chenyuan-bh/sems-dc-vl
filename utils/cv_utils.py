#!/usr/bin/env python
"""
Adapted from TFMatch:
https://github.com/lzx551402/tfmatch
"""
...
class MatcherWrapper(object):
    ...
    def get_matches(self, feat1, feat2, cv_kpts1, cv_kpts2, ratio=None, cross_check=True, err_thld=4, ransac=True,
                    info='', display=True):
        ...
        init_matches1 = self.matcher.knnMatch(feat1, feat2, k=2)  # feat might be nan
        ...
        if len(good_matches) == 0:
            ...
            return None, None
            # raise Exception("No match points!")
            # exit(-1)
        ...
        if ransac:
            if ransac == 'h':
                H, mask = cv2.findHomography(good_kpts1, good_kpts2, cv2.RANSAC, err_thld)
            else:
            ...
            mask = np.ones((len(good_matches),), dtype=np.int32)
            if display:
    ...
    def draw_matches(self, img1, cv_kpts1, img2, cv_kpts2, good_matches, mask,
                     match_color=(255, 255, 0), pt_color=(0, 0, 255)):
        """Draw matches."""
        ...
        # if not isinstance(mask,np.ndarray):return -1
        if mask is not None:  # (mask!=None).all()
            mask = mask.ravel().tolist()
        if good_matches == None:
            good_matches = []
            for i in range(len(cv_kpts1)):  # corrs
        ...
        display = cv2.drawMatches(img1, cv_kpts1, img2, cv_kpts2, good_matches,
                                  ...
                                  singlePointColor=pt_color, matchesThickness=2,
                                  matchesMask=mask, flags=2)  # flag 2
        return display

    def get_matches_H(self, feat1, feat2, cv_kpts1, cv_kpts2, ratio=None, cross_check=True, err_thld=4, ransac=True,
                      info=''):
        """Compute putative and inlier matches.
        Args:
            feat: (n_kpts, 128) Local features.
        ...
        if ransac:
            ...
            print(info, 'n_putative', len(good_matches))
            trans = None
        return good_matches, mask, trans

    def draw_seg_matches(self, seg1, cv_kpts1, seg2, cv_kpts2, good_matches, mask,
                         match_color=(255, 255, 0), pt_color=(0, 0, 255), resize=1):
        cmap = np.array([
            [128, 64, 128],  # road
            [152, 251, 152],  # terrain
        ...
        if type(seg1) is torch.Tensor and type(seg2) is torch.Tensor:
            seg1 = seg1.argmax(dim=0).detach().cpu().numpy()
            seg2 = seg2.argmax(dim=0).detach().cpu().numpy()
        seg1 = cmap[seg1]
        seg2 = cmap[seg2]
        if resize != 1:
            seg1 = cv2.resize(seg1, (0, 0), fx=resize, fy=resize, interpolation=cv2.INTER_LINEAR)
            seg2 = cv2.resize(seg2, (0, 0), fx=resize, fy=resize, interpolation=cv2.INTER_LINEAR)
        ...
        if good_matches == None:
            good_matches = []
            for i in range(len(cv_kpts1)):  # correspondings
                match = cv2.DMatch(_queryIdx=i, _trainIdx=i, _distance=0)
                good_matches.append(match)
        display = cv2.drawMatches(seg1, cv_kpts1, seg2, cv_kpts2, good_matches,
                                  None,
                                  matchColor=match_color,
                                  singlePointColor=pt_color, matchesThickness=2,
                                  matchesMask=mask, flags=2)  # flag 2
